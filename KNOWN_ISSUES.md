...
## 4. [RESUELTO] El reporte proactivo de un lote mandaba imágenes 4K (TV-only) por Telegram
...
**Qué pasó:** la excepción escapó de `_send_batch_report` sin capturarse, se propagó a través de `Application.create_task` hasta `global_error_handler` (mismo mecanismo ya validado en dev_plan §3.1 para excepciones reales del corredor de fondo) — el proceso no se cayó, quedó registrada en el log, pero el usuario nunca recibió el álbum de fotos de ese lote (el texto sí llegó, se manda antes). `batch.status` se quedó en `'running'` (nunca llegó a `'reported'`); `reconcile_batches_on_startup` (dev_plan §3.3) sí lo recogió correctamente como no-terminal en el reinicio siguiente del bot y reintentó mandar el reporte — confirmando en vivo que el mecanismo de recuperación de 3.3 sigue funcionando incluso ante este fallo, aunque el reintento en sí fallaba exactamente igual (mismo archivo, mismo tamaño) hasta que se aplicó el fix de abajo.

**Causa raíz real (no el tamaño en sí):** el sistema nunca debería mandar una imagen 4K por Telegram en primer lugar — 4K es exclusivamente el formato de entrega a las TVs (PRD §7.7, "two-pass resolution": 1K para iterar/mostrar, 4K solo para el resultado final en pantalla física). `batch_item` solo tenía una columna `image_id`, que la finalización sobreescribía con el `image_id` 4K en cuanto tenía éxito — el id 1K del draft se perdía. `summarize_batch` leía esa misma columna tal cual, así que para cualquier lote ya `finalized`, el único id disponible para el reporte era el 4K. Esto era inconsistente con el flujo de pieza suelta (`_panels_album`), que siempre manda los `image_id` capturados de los argumentos de `compose_preview` (el draft 1K, nunca sobreescrito) — nunca tuvo este problema.

**Fix aplicado (mismo día):** `batch_item` gana `draft_image_id` y `batch_day` gana `draft_wide_image_id` (columnas nuevas, migración vía `ALTER TABLE` guardado, mismo patrón que `policy_rejection`/`chat_id`/`report_text_sent`) — se escriben **una sola vez**, en el momento del draft, y la finalización nunca las toca (`_execute_item_update`/`_execute_wide_update` en `batch_store.py` solo incluyen la columna `draft_*` en el `UPDATE` cuando el caller la pasa explícitamente, para que una llamada de finalización que no la pase no borre el draft ya guardado). `summarize_batch` expone `draft_image_id` por panel y `draft_wide_image_id` por día. `_batch_report_albums` (`telegram_bot.py`) usa exclusivamente estos ids 1K, nunca `image_id`/`wide_image_id`. Para un día split, el reporte manda **una sola foto** de la composición ancha 1K completa (`draft_wide_image_id`) + el panel 50 — 2 fotos contra las 3 de un día independiente (decisión deliberada: más simple que partir también el draft, y arguiblemente mejor vista previa que dos recortes truncados).

**Tests:** `tests/test_engine_batch_store.py` (+5: persistencia de `draft_image_id`/`draft_wide_image_id`, no-sobreescritura por una llamada de finalización sin el campo — el caso central del bug —, migración sobre una base sin las columnas). `tests/test_engine_batch.py` (+3: `_finalize_item`/`_finalize_split_day` preservan el draft tras sobreescribir `image_id`/`wide_image_id`, `summarize_batch` expone el shape nuevo). `tests/test_bot_telegram.py` (+2: `_batch_report_image_ids` usa siempre los ids 1K nunca los 4K; un día split manda exactamente 2 fotos, no 3) — los 8 tests de reporte ya existentes se actualizaron para el shape nuevo de `summarize_batch` sin cambiar su comportamiento esperado.

**Estado:** resuelto, 2026-07-18. `batch_id=batch_2a185ece` (el lote real de este repro) se marcó manualmente `status='reported'` en `data/batch.sqlite3` tras confirmar que su trabajo real ya estaba completo (6/6 `uploaded`, rotación en las 3 TVs) — su reporte específico nunca se reenviará retroactivamente, pero cualquier lote nuevo ya usa el fix.
