...
A system that lets you direct, by conversation, the AI-generated art shown on three Samsung Frame TVs (two 43" verticals + one 50" horizontal) in a house. Full spec lives in `docs/prd.md`; the iteration-by-iteration build plan (what's done, what's next, and why) lives in `docs/dev_plan.md` — **read `docs/dev_plan.md` before starting new work** to see current status and avoid re-doing decisions already made. `KNOWN_ISSUES.md` tracks real defects found in manual testing that aren't blocking but need a product decision later.
