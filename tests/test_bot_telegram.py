...
def test_build_application_registers_a_single_whitelisted_handler():
    ...
    assert handler.callback is handle_message
    assert handler.filters.base_filter.user_ids == frozenset({111, 222})
    assert isinstance(handler.filters.and_filter, type(tg_filters.TEXT))
...
def test_build_application_with_empty_whitelist_matches_no_one():
    ...
    handlers = [h for group in app.handlers.values() for h in group]
    assert handlers[0].filters.base_filter.user_ids == frozenset()
...
def _make_update_and_context(runner, session_service, chat_id, text="hola"):
    ...
    context = SimpleNamespace(
        application=SimpleNamespace(
            bot_data={"runner": runner, "session_service": session_service}
        )
    )
...
def test_first_message_creates_session_and_replies_with_agent_text():
    ...
    update.message.reply_text.assert_awaited_once_with("listo, img_123")
    session = asyncio.run(
        session_service.get_session(app_name=APP_NAME, user_id="42", session_id="42")
    )
...
def test_multiple_text_events_are_concatenated_into_the_reply():
    ...
    update.message.reply_text.assert_awaited_once_with("primero\nsegundo")
...
def test_events_without_text_parts_contribute_nothing_to_the_reply():
    ...
    update.message.reply_text.assert_awaited_once_with("listo")
...
def test_non_text_messages_are_excluded_by_the_registered_filter():
    ...
    handler = [h for group in app.handlers.values() for h in group][0]
    assert isinstance(handler.filters.and_filter, type(tg_filters.TEXT))
