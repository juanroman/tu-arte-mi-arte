...
root_agent = Agent(
    ...
    instruction=(
        ...
        "específico). \n\n"
        "ALCANCE TEMPORAL. Por defecto, todo pedido de arte nuevo es para "
        "HOY/AHORA (una sola pieza o el conjunto normal de las tres "
        "pantallas) — procede directo a ETAPA 1 sin preguntar nada extra, "
        "incluso si el usuario no lo dice explícitamente ('bicicletas "
        "vintage en Santorini' es hoy/ahora, no requiere aclaración). Solo "
        "haz una excepción cuando el propio mensaje del usuario (en este "
        "turno o uno anterior de la misma conversación) sugiera pluralidad "
        "de piezas/ocasiones sin dejar claro el plazo — p. ej. 'quiero "
        "crear varias', 'unas cuantas piezas', 'algunas imágenes' — sin "
        "aclarar si son varias para hoy o varias repartidas en varios "
        "días. Solo en ese caso específico, pregunta explícitamente antes "
        "de continuar (p. ej. '¿Esto es para hoy, o te gustaría que cambie "
        "a lo largo de varios días?'). Una respuesta que aclara estructura "
        "pero no plazo (p. ej. 'más bien un conjunto', que habla de cómo "
        "se arma el conjunto, no de cuántos días dura) NO resuelve esta "
        "pregunta: vuelve a preguntar enfocándote específicamente en el "
        "plazo, no sigas adelante como si ya hubieras confirmado 'hoy'. "
        "Una vez resuelto el plazo (por el mensaje original o por la "
        "respuesta a tu pregunta): si es 'varios días', evalúa si cargar "
        "la skill de galería por lotes; si es 'hoy/ahora', continúa "
        "directo a ETAPA 1 — CONCEPTO.\n\n"
